...
func (s *Service) Initialize() {
	...
	if !s.Config.DisableParamValidation.Get() {
...
func (s *Service) buildEndpoint() {
	if s.Config.Endpoint.Get() != "" {
		s.Endpoint = s.Config.Endpoint.Get()
	} else {
		s.Endpoint, s.SigningRegion =
			endpoints.EndpointForRegion(s.ServiceName, s.Config.Region.Get())
	}

	if s.Endpoint != "" && !schemeRE.MatchString(s.Endpoint) {
		scheme := "https"
		if s.Config.DisableSSL.Get() {
			scheme = "http"
		}
...
func (s *Service) AddDebugHandlers() {
	out := s.Config.Logger
	if s.Config.LogLevel.Get() == 0 {
		return
	}

	s.Handlers.Send.PushFront(func(r *Request) {
		logBody := r.Config.LogHTTPBody.Get()
...
func (s *Service) MaxRetries() uint {
	if s.Config.MaxRetries.Get() < 0 {
		return s.DefaultMaxRetries
	}
	return uint(s.Config.MaxRetries.Get())
}
